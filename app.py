...
from database import (init_db,save_chat_message,
                      list_conversations,create_or_update_conversation,get_chat_history)
...
#create file directory if it does not exit
...
async def upload_document(
    ...
    try:
        ...
        return JSONResponse({
            "success": True,
            "message": f"Uploaded {result['filename']} and created {result['chunks']} chu...",
        })
...
def should_stream_chunk(chunk,metadata)->bool:
    ...
    addtional_kwargs = getattr(chunk, "addtional_kwargs", {}) or {}
